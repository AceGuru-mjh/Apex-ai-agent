...
/**
 * 将流中的元素按指定大小分前分组）， *
 * @param size 每个块的大小，* @return 一个新的Stream，它发出元素列表 (，， * @throws IllegalArgumentException 如果 `size` 不是正数据*/
...
fun Stream<String>.splitBy(plugins: List<StreamPlugin>): Stream<StreamGroup<StreamPlugin?>> {
    ...
    // 创建一个包装的Stream，附带委托功能   val delegatingStream =
    ...
    // 将字符串流转换为字符为   return delegatingStream
            .flatMap { str ->
                stream {
                    for (char in str) {
                        emit(char)
...
/**
 * 节流操作，在指定的时间窗口内仅发射最后一个元素与throttleFirst相反，throttleFirst保留窗口内第一个元索*
