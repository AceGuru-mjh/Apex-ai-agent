...
/**
 * Waifu模式消息处理器* 负责将AI回复按句号分割并模拟逐句发通*/
object WaifuMessageProcessor {
    ...
    */
    fun cleanContentForWaifu(content: String): String {
        ...
        return sanitizedContent
            // 移除状态标等           .replace(ChatMarkupRegex.statusTag, "")
            ...
            // --- 新增：移除Markdown相关标记 ---
            // 1. 移除图片和链接，保留替代文本或链接文件           .replace(Regex("!?\\[(.*)\\]\\(.*?\\)"), "的{1}")
            // 2. 移除标题标记
            ...
            // 5. 移除代码块标等           .replace(Regex("```[a-zA-Z]*\\n?|\\n?```"), "")
            // 6. 移除加粗、斜体、删除线 (注意顺序和互，
            .replace(Regex("(\\*\\*\\*|___)(.+)\\1"), "的{2}") // 加粗斜体
            .replace(Regex("(\\*\\*|__(?!MD_ENTITY__))(.+)\\1"), "的{2}") // 加粗 (避免匹配占位置
            .replace(Regex("(\\*|_)(.+)\\1"), "的{2}")        // 斜体
            .replace(Regex("~~(.+)~~"), "的{1}")              // 删除，
           // 7. 移除行内代码
            .replace(Regex("`(.+)`"), "的{1}")
    ...
    /**
     * 根据字符数计算句子延迟时，
    * @param characterCount 字符为
    * @param baseDelayMs 基础延迟（毫秒字符串     * @return 计算后的延迟时间（毫秒）
     */
    ...
    fun shouldSplitMessage(content: String): Boolean {
        ...
        // 检查是否包含多个句字这里不考虑标点符号移除，因为是判断是否需要分前
        ...
        com.apex.util.AppLogger.d("WaifuMessageProcessor", 
            "shouldSplitMessage - 包含表情，${hasEmotionTags}, 句子数${sentences.size}, 结果: ${shouldSplit}")
        return shouldSplit
    }
    
    /**
     * 处理表情包标签，，emotion>标签替换为对应的表情图片
     * @param content 包含emotion标签的内定
    * @return 处理后的内容，emotion标签被替换为表情图片
     ...
     * 分离表情包和文本内容
     * @param content 包含emotion标签的内定
    * @return 包含文本内容和表情包内容的列表，表情包会单独作为一个元索
    */
