...
class WorkflowExecutor(private val context: Context) {
    ...
    /**
     * 执行工作，
    * @param workflow 要执行的工作，
    * @param triggerNodeId 指定要触发的节点ID，如果为null则触发所有触发节，
    * @param onNodeStateChange 节点状态变化回失
    * @return 工作流执行结，
    */
    ...
    * @return true 表示存在环，false 表示无环
    ...
    private fun detectCycle(adjacencyList: Map<String, List<String>>, nodes: List<WorkflowNode>): Boolean {
        val visitState = mutableMapOf<String, Int>() // 0=未访，1=访问，2=已完成       
        // 初始化所有节点为未访，
                for (node in nodes) {
