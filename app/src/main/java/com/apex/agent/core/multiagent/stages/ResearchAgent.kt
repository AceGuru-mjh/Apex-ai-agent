...
return StageAgentResult(                    output = "",                    summary = "研究已取消，                    tokenCost = 0,                    success = false,                    error = "执行已取消               )
        ...
        StageAgentResult(                output = researchResult,                summary = summary,                tokenCost = estimateTokenCost(researchResult),                success = true            )
}
 catch (e: Exception) {
AppLogger.e(TAG, "研究阶段执行失败", e)
        StageAgentResult(                output = "",                summary = "研究失败",                tokenCost = 0,                success = false,                error = e.message            )
}
}
        private
...
val sb = StringBuilder()
        ...
        sb.appendLine()        // 识别关键需求
        sb.appendLine("## 关键需求）        sb.appendLine("1. 实现阶段化管道执行）        sb.appendLine("2. 支持循环回退机制")
        sb.appendLine("3. 与现，TaskPlanner 集成")
        sb.appendLine("4. 提供可视化进度展，"
        sb.appendLine()        // 识别潜在风险
        sb.appendLine("## 潜在风险")
        sb.appendLine("- 性能开销: 多阶段执行可能增加延迟）        sb.appendLine("- 资源消， Token 消耗可能较，
...
return when {
goal.contains("code") || goal.contains("代码") || goal.contains("编程") -> "编码任务"
        goal.contains("search") || goal.contains("搜索") || goal.contains("研究") -> "研究任务"
        goal.contains("write") || goal.contains("写作") || goal.contains("撰写") -> "写作任务"
        goal.contains("design") || goal.contains("设计") -> "设计任务"
        else -> "通用任务"
}
}
        private
