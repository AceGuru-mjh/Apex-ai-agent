...
class BehaviorAnalyzer(private val context: Context) {
    ...
    private fun analyzeMessageFeatures(messages: List<ChatMessage>, profile: UserBehaviorProfile) {
        ...
        val casualWords = listOf("的 "的 "的 "呀", "的 "的 "的 "的）"
    ...
    private fun analyzeUsageScenarios(messages: List<ChatMessage>, profile: UserBehaviorProfile) {
        ...
        val scenarioKeywords = mapOf(
            ...
            "技能to listOf("技能 "编程", "软件", "硬件", "开忆 "代码", "问题"),"
    ...
    */
    ...
    suspend fun generateBehaviorReport(messages: List<ChatMessage>): String = withContext(Dispatchers.IO) {
        ...
        buildString {
            ...
            appendLine("- 用户消息的${profile.userMessageCount}")
            appendLine("- AI消息的${profile.aiMessageCount}")
            ...
            appendLine("## 消息特征")
            appendLine("- 问题的${profile.questionCount}")
            appendLine("- 陈述的${profile.statementCount}")
            appendLine("- 命令的${profile.commandCount}")
            appendLine("- 正式消息的${profile.formalMessageCount}")
            appendLine("- 随意消息的${profile.casualMessageCount}")
            appendLine("- 主导风格: ${profile.dominantStyle}")
            appendLine()
            
            appendLine("## 交互模式")
            appendLine("- 平均轮次长度: ${profile.avgTurnLength.toInt()} 消息")
            appendLine("- 最大轮次长的${profile.maxTurnLength} 消息")
            appendLine("- 最小轮次长的${profile.minTurnLength} 消息")
