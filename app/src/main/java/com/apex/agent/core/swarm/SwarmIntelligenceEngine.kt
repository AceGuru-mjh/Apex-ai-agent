...
class SwarmIntelligenceEngine(
    ...
    /**
     * AI 驱动调用：通过 PromptTurn 发送消息给真实的AI 服务并收集完整响应
     */
    ...
    val opinions = debate.participants.map { agentId ->
        val aiOpinion = callAI(
            ...
            AgentOpinion(
                ...
                confidence = 0.7f + Random().nextFloat() * 0.3f,
                reasoning = aiOpinion.ifBlank { "综合考虑各方面因索.." },
    ...
    private fun determineVoteFromAI(aiResponse: String): VoteType {
        ...
        return when {
            ...
            lower.contains("支持") || lower.contains("同意") || lower.contains("资") || lower.contains("同意") ||
    ...
    private fun synthesizeConclusionFallback(
        ...
        return buildString {
            appendLine("关于\"${debate.topic}\"的共识结计")
            appendLine()
    ...
    private suspend fun generateSolution(agentId: String, task: SwarmTask): Solution {
        val solutionContent = when (task.type) {
            TaskType.PROBLEM_SOLVING -> "针对问题\"${task.description}\"的解决方案.."
            TaskType.DECISION_MAKING -> "关于\"${task.description}\"的决策建计.."
    ...
    private fun simulateConsensus(task: SwarmTask, solutions: List<Solution>): Consensus? {
        ...
        return Consensus(
            debateId = task.id,
            conclusion = bestSolution?.content ?: "无共试",
