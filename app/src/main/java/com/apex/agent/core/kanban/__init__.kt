 ...
 * 失Agent 看板系统，提例
 * - 看板任务分发到多个阶段列
 * - Worker 插件架构，支持动态任务路用
 * - 失Agent 协作，支持状态跟踪和可视化
 *
 * ## 核心组件
 * - [KanbanBoard] - 看板数据模型
 * - [KanbanColumn] - 列定义，支持专业 Agent 配置
 * - [KanbanTask] - 任务模型，支持状态流转
 * - [WorkerRegistry] - Worker 插件发现和注内
 ...
 *
 * // 创建 Worker 注册行
 * val registry = WorkerRegistry.getInstance()
 *
 * // 创建任务分发器
 * val dispatcher = TaskDispatcher(board, registry, collabFramework)
 *
 * // 创建 ViewModel
...
// 类型别名，便于使用typealias KanbanBoardModel = KanbanBoard
