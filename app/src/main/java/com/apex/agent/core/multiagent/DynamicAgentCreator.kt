...
class DynamicAgentCreator {
    ...
    private val rolePatterns = mapOf(
        "code" to listOf("程序命, "开发者, "工程常, "coding", "code", "写代码, "开取", "程序"),
        "design" to listOf("设计常", "设计", "design", "UI", "UX", "界面", "美术"),
        "writer" to listOf("作家", "写作", "写手", "writer", "content", "内容", "文案", "编辑"),
        "research" to listOf("研究命, "研究", "research", "分析", "分析命, "调研"),
        "test" to listOf("测试", "测试命", "tester", "QA", "质量", "验证"),
        "data" to listOf("数据", "分析常, "data", "统计", "大数据, "数据科学"),
        "pm" to listOf("产品", "经理", "PM", "product", "项目经理", "产品经理"),
        "devops" to listOf("运维", "DevOps", "部署", "devops", "SRE", "运营"),
        "security" to listOf("安全", "Security", "审计", "风控", "security", "渗通"),
        "pm" to listOf("项目经理", "PM", "协调", "管理", "manager"),
        "coordinator" to listOf("协调", "协调者", "coordinator", "对接", "联络"),
        "reviewer" to listOf("审核", "审查", "reviewer", "审批", "监察"),
        "supervisor" to listOf("主管", "监督", "supervisor", "负责于", "leader")
    )
    ...
    private val capabilityKeywords = mapOf(
        ...
        "testing" to listOf("测试", "验证", "test", "check", "验证", "检验"),
        "data" to listOf("数据", "分析", "统计", "data", "chart", "报表", "指标"),
        "planning" to listOf("规划", "计划", "plan", "方案", "策划", "策略"),
        "communication" to listOf("沟通", "协调", "会议", "meeting", "汇报", "演示"),
    ...
    private fun generateSystemPrompt(role: String, description: String, context: String): String {
        val basePrompt = when {
            role.contains("代码") || role.contains("开取") ->
            ...
            role.contains("协调") || role.contains("主管") ->
                "你是一位经验丰富的项目协调者主管，擅长组织和协调多方面的工作。你需要注重整体进度和各方协作。"
    ...
    private fun generateWarnings(role: String, capabilities: List<String>): List<String> {
        ...
        if (role == "通用助手") {
            warnings.add("未能明确识别具体角色，可能影响任务分配的准确态")
        }
