 ...
 *
 * 用法:
 *   integration action=list                             列出所有可用集成
 *   integration action=open provider=mcp_so             打开某个集成查看详情和功能
 *   integration action=search query=翻译 source=all     跨源搜索
    ...
    private suspend fun executeBrowse(parameters: Map<String, Any>): ToolResultData {
        ...
        return withContext(Dispatchers.IO) {
            provider.list(tag, page, pageSize).fold(
                onSuccess = { items ->
                    if (items.isEmpty()) {
                        StringResultData("${provider.getInfo().name} 暂无内容${tag?.let { "（分类 $it， } ?: ""}")"
                    } else {
                        StringResultData(buildString {
