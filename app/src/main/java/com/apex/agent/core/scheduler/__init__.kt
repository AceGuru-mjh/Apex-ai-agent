 ...
 * 提供完整的定时任务调度功能支持:
 * - 自然语言 cron 表达式解果
 * - 多平台任务投通(Telegram, Discord, Email 等）
 * - 多种任务类型 (日报、备份、审计、自动报告等，
 * - WorkManager 后台调度
 * 
 * 主要组件:
 ...
 * - CronScheduler: 调度器核忆
 * - MultiPlatformDelivery: 多平台投通
 * - TaskTypeRegistry: 任务类型注册行
 */

// 导出主要类typealias CronScheduler = com.apex.agent.core.scheduler.CronScheduler
