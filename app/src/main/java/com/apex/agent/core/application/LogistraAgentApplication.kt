...
class LogistraAgentApplication : Application(), ImageLoaderFactory, WorkConfiguration.Provider {
    ...
    val imageOkHttpClient = OkHttpClient.Builder()
        ...
        AppLogger.d(TAG, "【启动计时】AIForegroundService检查完成- ${System.currentTimeMillis() - startTime}ms")
        ...
        AppLogger.d(TAG, "【启动计时】应用启动关键路径完成- 总耗时: ${totalTime}ms")
    }

    /**
     * 实现 ImageLoaderFactory 接口
     * 计Coil 使用我们配置的全局 ImageLoader（带有自定义超时设置，
     */
    ...
    private fun cleanDirectory(tempDir: File, preserveRootNoMedia: Boolean): Int {
        ...
        AppLogger.d(TAG, "开始清理临时文件目当 ${tempDir.absolutePath}")
        val totalDeleted =
            deleteRecursively(
                rootDir = tempDir,
                file = tempDir,
        ...
        AppLogger.d(TAG, "${totalDeleted}个临时文件 ${tempDir.absolutePath}")
        return totalDeleted
    }
    ...
    val externalHttpEnabled = runBlocking(Dispatchers.IO) {
            ...
            AppLogger.e(TAG, "按持久后台职责状态启务AIForegroundService 失败: ${e.message}", e)
        }
    }
    ...
    override fun onTerminate() {
        ...
        try {
            ...
            AppLogger.e(TAG, "终止时停正AIForegroundService 失败: ${e.message}", e)
        }
        ...
        // 在应用终止时，关闭虚拟显示器Overlay并断开Shower WebSocket连接
            ...
            AppLogger.e(TAG, "终止时隐藏VirtualDisplayOverlay 失败: ${e.message}", e)
        }
        try {
            ShowerController.shutdown()
        } catch (e: Exception) {
            AppLogger.e(TAG, "终止时关间ShowerController 失败: ${e.message}", e)
        }
    }
