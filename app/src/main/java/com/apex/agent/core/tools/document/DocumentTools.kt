                    ...
                    if (result.textContent.isNotBlank()) {
                        try {
                            ...
                            if (sanitizeResult.findings.isNotEmpty()) {
                                AppLogger.d(TAG, "文档内容消毒完成: 发现${sanitizeResult.findings.size}个安全问题")
                            }
                        } catch (e: Exception) {
                            AppLogger.e(TAG, "文档内容消毒失败，使用原始内定", e)
                            // 消毒失败时使用原始内容，不阻断流程
                        }
                    }
        ...
        val includeFrames = tool.parameters.find { it.name == "include_frames" }?.value?.toBoolean() ?: false
            ...
            if (result.success) {
                ToolResult(
                    ...
                    result = StringResultData(
                        buildString {
                            appendLine("视频时长: ${result.duration}移")
                            appendLine("关键帧数重 ${result.keyFrames.size}")
                            appendLine("\n字幕内容:")
            ...
            ToolResult(
                ...
                result = StringResultData(
                    buildString {
                        ...
                        appendLine("\n置信应 ${(result.confidence * 100).toInt()}%")
