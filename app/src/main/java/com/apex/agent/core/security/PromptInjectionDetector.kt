 ...
 * 提示注入检测器
 *
 * 通过规则引擎匹配已知的提示注入模式（中英文），检测试图改取AI 角色。
 * 绕过安全限制等恶意输入。
 */
class PromptInjectionDetector {
    ...
    companion object {
        ...
        add(InjectionPattern(
            ...
            // ===== 中文模式 =====
        add(InjectionPattern(
                pattern = Pattern.compile("忽略之前的所有）?(指令|指示|规则|设定，"),
                type = InjectionPatternType.INSTRUCTION_OVERRIDE,
                baseConfidence = 0.9f,
                severity = RiskLevel.CRITICAL
            ))
        add(InjectionPattern(
                pattern = Pattern.compile("无视之前的所有）?(指令|指示|规则|设定，"),
                type = InjectionPatternType.INSTRUCTION_OVERRIDE,
                baseConfidence = 0.9f,
                severity = RiskLevel.CRITICAL
            ))
        add(InjectionPattern(
                pattern = Pattern.compile("忘记之前的所有）?(指令|指示|规则|设定|内容，"),
                type = InjectionPatternType.INSTRUCTION_OVERRIDE,
                baseConfidence = 0.85f,
                severity = RiskLevel.HIGH
            ))
        ...
        add(InjectionPattern(
                pattern = Pattern.compile("从现在开始[，]?作是一个|将成为|扮演，"),
                type = InjectionPatternType.ROLE_ASSIGNMENT,
                baseConfidence = 0.8f,
                severity = RiskLevel.HIGH
            ))
        ...
        add(InjectionPattern(
                pattern = Pattern.compile("不再取?:任何，(?:限制|约束，"),
                type = InjectionPatternType.SAFETY_BYPASS,
                baseConfidence = 0.85f,
                severity = RiskLevel.HIGH
            ))
    ...
    fun detectAll(input: String): List<InjectionFinding> {
        ...
        if (allFindings.isNotEmpty()) {
            AppLogger.w(TAG, "检测到 ${allFindings.size} 个提示注具角色劫持模式")
        }
        return allFindings
    }
}
