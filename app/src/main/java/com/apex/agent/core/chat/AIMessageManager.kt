 ...
 * 设计原则:
 * - **无状态: 本身不持有任何特定聊天的状态。所有需要的数据都通过方法参数传入，* - **职责明确**: 仅处理与AI服务的交互，UI更新和数据持久化由调用方负责，* - **封装逻辑**: 内部封装了与AI交互的策略，如是否需要总结、如何从历史中提取记忆等着 */
object AIMessageManager {
    private const val TAG = "AIMessageManager"
    // 聊天总结的消息数量阈， 移除硬编码，改用动态设置   // private const val SUMMARY_CHUNK_SIZE = 4
     ...
     * @param messageText 用户输入的原始文本，     * @param attachments 附件列表，
    * @param enableMemoryQuery 是否允许AI查询记忆，
    * @param enableWorkspaceAttachment 是否启用工作区附着功能的
    * @param workspacePath 工作区路径，     * @param workspaceEnv 工作区环境，     * @param replyToMessage 回复消息，
    ...
    * @param chatHistory 完整的聊天历史记录，     * @param workspacePath 当前工作区路径，     * @param promptFunctionType 提示功能类型，
    * @param enableThinking 是否启用思考过程，     * @param thinkingGuidance 是否启用思考引导，     * @param enableMemoryQuery 是否允许AI查询记忆，
    * @param maxTokens 最大token数量，
    * @param tokenUsageThreshold token使用阈值，     * @param onNonFatalError 非致命错误回调用     * @param onTokenLimitExceeded token限制超出回调用
    ...
    */
    suspend fun sendMessage(
        ...
        val afterMediaLinkCount = memoryForRequest.count { MediaLinkParser.hasMediaLinks(it.content) }
            if (beforeMediaLinkCount != afterMediaLinkCount) {
                AppLogger.d(
                    TAG,
                    "历史音视频裁剪生成limit=${maxMediaHistoryUserTurns}, before=${beforeMediaLinkCount}, after=${afterMediaLinkCount}"
                )
            }
            logMessageTiming(
    ...
    val prepareRequestStartTime = messageTimingNow()
        val responseStream = enhancedAiService.sendMessage(
                ...
                tokenUsageThreshold = tokenUsageThreshold,
                onNonFatalError = onNonFatalError,
                onTokenLimitExceeded = onTokenLimitExceeded, // 传递回失
    ...
    * 这会同时尝试取消插件接管执行（如果正在进行）和底层的AI流，     */
    ...
    fun cancelOperation(chatId: String) {
        ...
        AppLogger.d(TAG, "AI操作取消请求已发通chatId=${chatKey}")
    }
    ...
    *
     * @param enhancedAiService AI服务实例，
    * @param messages 需要总结的消息列表，     * @param autoContinue 是否为自动续写模式，如果是则在总结消息尾部添加续写提示，
    ...
    */
     ...
     * 判断是否应该生成对话总结，
    *
     * @param messages 完整的消息列表，     * @param currentTokens 当前上下文的token数量，
    ...
    */
    fun shouldGenerateSummary(
        ...
        AppLogger.d(TAG, "未达到生成总结的条，Token使用于${if (maxTokens > 0) currentTokens.toDouble() / maxTokens else 0.0}")
        return false
    }
