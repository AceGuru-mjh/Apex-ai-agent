...
sb.appendLine("## 迭代信息")            sb.appendLine("当前循环次数: ${loopCount}")            sb.appendLine("本次实现针对上一轮验证失败的问题进行了修复）            sb.appendLine()"
        ...
        sb.appendLine("## 实现内容")        sb.appendLine()        sb.appendLine("### 1. 核心数据结构，"
       sb.appendLine("```kotlin")        sb.appendLine("data"
...
")        sb.appendLine("```")        sb.appendLine()        sb.appendLine("### 3. 阶段 Agent")        sb.appendLine("- ResearchAgent: 信息收集")        sb.appendLine("- PlannerAgent: 任务规划")        sb.appendLine("- ImplementerAgent: 代码实现")        sb.appendLine("- ReviewerAgent: 代码审查")        sb.appendLine("- ValidatorAgent: 验证测试")        sb.appendLine()        sb.appendLine("### 4. 集成方案，
       sb.appendLine("- TaskPlanner: 复杂度判断和自动启用")        sb.appendLine("- CollaborationEngine: 状态协调）        sb.appendLine("- UI ，进度展示")        return sb.toString()"
}
    private
