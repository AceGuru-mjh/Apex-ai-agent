...
class MultiModalFusionEngine(
    ...
    companion object {
        ...
        if (text.contains("需要", ignoreCase = true) || text.contains("应该", ignoreCase = true)) {
            insights.add(Insight(
                type = InsightType.ACTION_ITEM,
                content = "检测到行动需求 ${text}",
                sourceModal = ModalType.TEXT,
                confidence = 0.85f
            ))
        }
        ...
        return listOf(Insight(
            type = InsightType.FACTS,
            content = "语音输入 - 时长: ${modalData.metadata["duration"]}移",
        ...
        return listOf(Insight(
            type = InsightType.FACTS,
            content = "视频输入 - 时长: ${modalData.metadata["duration"]}移 分辨现 ${modalData.metadata["resolution"]}",
        ...
        if (textModals.isNotEmpty() && imageModals.isNotEmpty()) {
            insights.add(Insight(
                ...
                content = "检测到文本+图像组合输入，可能需要图像理解配合文本分果",
                sourceModal = ModalType.TEXT,
                confidence = 0.8f,
        ...
        return buildString {
            appendLine("多模态融合推理过程")
        appendLine("输入模态 ${input.modalities.joinToString { it.type.name }}")
        appendLine()
        appendLine("提取的洞对")
        ...
        FusionReport(
            inputId = input.id,
            summary = "多模态输入处理完成，具${input.modalities.size} 种模态，生成 ${result.insights.size} 条洞对",
