...
object ToolExecutionManager {
    ...
    /**
     * ，AI 响应中提取工具调用了     * @param response AI 的响应字符串，
    * @return 检测到的工具调用列表，     */
    ...
    /**
     *
     * 执行工具调用，包括权限检查、并且串行执行和结果聚合，
    * @param invocations 要执行的工具调用列表，
    * @param toolHandler AIToolHandler 的实例，     * @param packageManager PackageManager 的实例，     * @param collector 用于实时输出结果，StreamCollector，
    * @return 所有工具执行结果的列表，
    */
    ...
    */
    ...
    val toolResultStatusContent =
        ...
        val combinedResultString = collectedResults.joinToString("\n") { res ->
                ...
                // 尝试使用新的工具适配置
                AppLogger.d(TAG, "尝试使用工具适配器执行工具${toolName}")
    ...
    */
    ...
    val toolResultStatusContent =
            ...
            AppLogger.e(TAG, "工具适配器执行失败${invocation.tool.name}", e)
            val errorMessage = "工具适配器执行失败${e.message}"
