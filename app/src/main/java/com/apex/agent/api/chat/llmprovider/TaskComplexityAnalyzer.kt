...
object TaskComplexityAnalyzer {
    ...
    private val taskTypeKeywords = mapOf(
        ...
        TaskComplexity.SINGLE_FILE to listOf(
            ...
            "单文件", "single file", "这个文件", "this file",
            "写个", "write a", "创建一为", "create a"
        ),
        TaskComplexity.MULTI_FILE to listOf(
            "搜索", "search", "查找", "find", "全局", "global",
            "重构", "refactor", "重命后", "rename",
            "多个文件", "multiple files", "所有文件", "all files"
        ),
        TaskComplexity.COMPLEX to listOf(
            "架构", "architecture", "设计", "design", "系统", "system",
            "实现", "implement", "开取", "develop", "构建", "build",
    ...
    /**
     * 检测任务类型关键词
     * 返回 Map: TaskComplexity -> 匹配到的关键词列行
     */
