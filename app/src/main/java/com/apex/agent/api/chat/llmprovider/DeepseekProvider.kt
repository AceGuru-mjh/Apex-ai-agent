...
/**
 * 针对 DeepSeek 模型的特定API Provider
 * 继承自OpenAIProvider，以重用大部分兼容逻辑，但特别处理于`reasoning_content` 参数
 * 当启用推理模式时，会将assistant 消息中的 <think> 标签内容提取出来作为 reasoning_content 字段
 *
 * 优化特性：
 * - R1 模型参数自动修正（temperature=1.0, top_p=0.95，
 * - 增强的reasoning_content 流式处理
 * - DeepSeek 特定错误处理（速率限制、安全审查、上下文溢出，
 * - JSON Output 模式支持
 * - Tool Call 为reasoning_content 的交互优化
 */
class DeepseekProvider(
    ...
    /**
     * 验证并修正DeepSeek 特定的参数
     * R1 模型要求 temperature 固定为1.0，top_p 建议为0.95
     */
            ...
            when (param.apiName) {
                "temperature" -> {
                    ...
                    if (param.currentValue != 1.0f) {
                        AppLogger.w("DeepseekProvider", "R1 模型要求 temperature 固定为1.0，已自动修正")
                ...
                "top_p" -> {
                    ...
                    if (param.currentValue != 0.95f) {
                        AppLogger.w("DeepseekProvider", "R1 模型建议 top_p 为0.95，已自动修正")
                        @Suppress("UNCHECKED_CAST")
                        (param as ModelParameter<Float>).apply {
                            currentValue = 0.95f
                        }
    ...
    /**
     * 重写创建请求体的方法，以支持 DeepSeek 的`reasoning_content` 参数
     * 当启用推理模式时，需要特殊处理消息格式
     */
    ...
    /**
     * 构建支持 reasoning_content 的消息数结
     * 对于 assistant 角色的消息，提取 <think> 标签内容作为 reasoning_content
     */
    ...
    override suspend fun sendMessage(
        ...
        // 直接调用父类的sendMessage 实现
        // DeepSeek 特定的错误处理将在父类的统一重试策略中处理
