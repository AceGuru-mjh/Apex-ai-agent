 ...
 * ### 注意事项
 *
 * - 默认值为 `false`，需要显式启务* - 启用后会自动添加 `tools` ，`tool_choice` 到请求体
 * - 流式响应中也支持增量工具调用数据的处理*
...
open class OpenAIProvider(
    ...
    * @param text 要处理的文本内容
    ...
    fun buildContentField(context: Context, text: String): Any {
        ...
        if (audioLinks.isNotEmpty() && !supportsAudio) {
            AppLogger.w("AIService", "检测到音频链接，但当前Provider不支持音频多模态输入，已移除音频。原始文本长，${text.length}, 处理器${textWithoutLinks.length}")
        }
        if (videoLinks.isNotEmpty() && !supportsVideo) {
            AppLogger.w("AIService", "检测到视频链接，但当前Provider不支持视频多模态输入，已移除视频。原始文本长，${text.length}, 处理器${textWithoutLinks.length}")
        }
        if (imageLinks.isNotEmpty() && !supportsVision) {
            AppLogger.w("AIService", "检测到图片链接，但当前Provider不支持图片处理，已移除图片。原始文本长，${text.length}, 处理器${textWithoutLinks.length}")
        }
    ...
    */
    ...
    // 向后兼容的快捷方法   private fun escapeXml(text: String) = XmlEscaper.escape(text)
    ...
    */
     ...
     * - 默认，incoming 视为增量追加，
    * - ，incoming 为前缀扩展快照（incoming startsWith existing），则直接替换为快照，
    */
    ...
    /**
     * 基于 canonical arguments ，fedLength 游标，向解析器仅喂入新增部分前
    */
    ...
    */
    ...
    private suspend fun processStreamingResponse(
        ...
        try {
            ...
            AppLogger.d(
                "AIService",
                "【发送消息】响应流处理完成，总块级${state.chunkCount}，输出token: ${tokenCacheManager.outputTokenCount}"
            )
    ...
    override suspend fun sendMessage(
        ...
        val responseStream = stream {
           ...
           tokenCacheManager.addOutputTokens(-tokenCacheManager.outputTokenCount)
            ...
            AppLogger.d(
                "AIService",
                "【发送消息】开始处理sendMessage请求，历史记录数据${chatHistory.size}，最后一条长，${chatHistory.lastOrNull()?.content?.length ?: 0}"
            )
