...
private fun AgentTab() {
    LazyColumn(
        ...
        item {
            Card(
                ...
                Column(modifier = Modifier.padding(20.dp)) {
                    ...
                    Text(
                        "智能AI助手 · 多Agent协作 · 爆发模式引擎",
                        fontSize = 14.sp,
                        color = MaterialTheme.colorScheme.onPrimaryContainer.copy(alpha = 0.8f)
                    )
                    Spacer(Modifier.height(12.dp))
                    Text(
                        "v1.0.0 — 多 APK 架构，9 个独立模块协同运行",
                        fontSize = 12.sp,
        ...
        item {
            Text("快捷功能", fontSize = 16.sp, fontWeight = FontWeight.SemiBold, color = MaterialTheme.colorScheme.onBackground, modifier = Modifier.padding(top = 8.dp))
        }
        items(
            listOf(
        ...
        items(apks) { apk ->
            Card(
                ...
                Row(
                    ...
                    Surface(
                        shape = RoundedCornerShape(12.dp),
                        color = if (apk.installed) Color(0xFF34A853) else Color(0xFFEA4335)
                    ) {
                        Text(
                            if (apk.installed) "已安装" else "未安装",
                            color = Color.White,
                            fontSize = 11.sp,
        ...
        items(modes) { mode ->
            Card(
                ...
                Row(
                    ...
                    Surface(
                        shape = RoundedCornerShape(12.dp),
                        color = mode.color.copy(alpha = 0.15f),
                        modifier = Modifier.size(48.dp)
                    ) {
                        Box(contentAlignment = Alignment.Center) {
                            Icon(mode.icon, contentDescription = null, tint = mode.color)
                        }
                    }
...
private fun DiagnosticsTab() {
    LazyColumn(
        ...
        item {
            Text("系统诊断", fontSize = 22.sp, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onBackground)
            Spacer(Modifier.height(8.dp))
        }
        items(
            listOf(
                DiagItem("应用版本", "v1.0.0 (versionCode 1)"),
